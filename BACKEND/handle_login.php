...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($stmt->num_rows === 1) {
        ...
        if (password_verify($password, $hashed_password)) {
            ...
            header("Location: ../FRONTEND/PAGES/4dashboard.html?success=" . urlencode('Login successful!'));
            exit();
        } else {
            header("Location: ../FRONTEND/PAGES/2login.html?error=" . urlencode('Invalid username or password.'));
            exit();
        }
    } else {
        header("Location: ../FRONTEND/PAGES/2login.html?error=" . urlencode('Invalid username or password.'));
